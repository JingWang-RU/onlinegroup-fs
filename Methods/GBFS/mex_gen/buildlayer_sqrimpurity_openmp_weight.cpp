                 ...
                 int nrhs, const mxArray *prhs[])
{
	...
	W = mxGetPr(prhs[5]);//Gradient weight
	
	// get additional args from matlab
    m_infty = mxGetPr(mxGetCell(prhs[6],0));
    l_infty = mxGetPr(mxGetCell(prhs[6],1));
	parents_labels = mxGetPr(mxGetCell(prhs[6],2));
	feature_costs = mxGetPr(mxGetCell(prhs[6],3));
			// added when returning loss for each node
	
	// get dimensions
	int numinstances = mxGetM(prhs[0]);
	int numnodes = mxGetN(mxGetCell(prhs[6],0));
	...
	for (int f=0; f<numfeatures; f++) {
	    ...
	    for (int j=f*numinstances; j<(f+1)*numinstances; j++) {
	        ...
	        // update variables
	        node->m_s += W[i]; // m_s is the number of data points encountered so far at that node
	        node->l_s += W[i] * l; // l_s is the total residual encountered so far at that node.
