function [args] = preprocesslayer_sqrimpurity_weight(data, options) 
	...
	for i=1:numnodes,
		m_infty(i) = sum(data.lapweight(data.n==i));
		l_infty(i) = sum(data.y(data.n==i).*data.lapweight(data.n==i),1);
	end
