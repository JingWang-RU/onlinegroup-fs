...
g = computeSlope(w,lambda/2,X'*(X*w-y),threshold);
    % check optimality
    ...
    free(maxpos) = 1;
    non_free = find(free>0);
    temp = X';
    free_xx = temp(non_free,:)*X(:,non_free);
...
% my_tmp = find(free_xx ~= XX(free>0, free>0));
% display('sum');
% disp(sum(my_tmp));
[wtemp fval exitflag output] = fminunc(@LassoObj,w(free > 0),options,free_xx,Xy(free>0),yy,lambda,threshold);
%      [wtemp fval exitflag output] = fminunc(@LassoObj,w(free > 0),options,XX(free>0,free>0),Xy(free>0),yy,lambda,threshold);
